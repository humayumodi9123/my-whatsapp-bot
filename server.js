...
// नया: उन लोगों की लिस्ट जिन्हें हमने मैसेज भेजा है
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        const phone = jid.split('@')[0]; // नंबर निकाल लिया
        
        // चेक करें कि क्या इस नंबर को हमने पहले मैसेज भेजा है?
        const sentList = getSentContacts();
        if (!sentList.includes(phone)) return; // अगर लिस्ट में नहीं है, तो रिप्लाई नहीं करेगा
...
app.get('/status', (req, res) => res.json({ connected: isConnected, qrCode: qrCodeData, autoReply: isAutoReplyEnabled }));
app.post('/toggle-autoreply', (req, res) => { isAutoReplyEnabled = req.body.enabled; res.json({ success: true, autoReply: isAutoReplyEnabled }); });
